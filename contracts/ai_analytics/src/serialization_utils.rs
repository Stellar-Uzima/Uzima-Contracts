...
impl SerializationUtils {
    ...
    where
        K: TryIntoVal<Env, Val> + Clone,
        V: TryIntoVal<Env, Val> + Clone,
    {
    ...
    pub fn validate_bytes_n<const N: usize>(
        ...
        soroban_sdk::log!(env, "Validating BytesN of length {}", N as u32);
...
impl<const N: usize> SafeSerialize for BytesN<N> {
    fn safe_serialize(&self, _env: &Env) -> Result<(), SerializationError> {
        SerializationUtils::validate_bytes_n(_env, self)
    }
}

impl SafeSerialize for Address {
    fn safe_serialize(&self, _env: &Env) -> Result<(), SerializationError> {
        SerializationUtils::validate_address(_env, self)
    }
}
