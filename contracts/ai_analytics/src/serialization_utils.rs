    ...
    fn as_error(&self) -> Self::Error {
        match self {
            SerializationError::CollectionTooLarge => {
                soroban_sdk::Error::from_contract_error(1000)
            }
            SerializationError::StringTooLong => {
                soroban_sdk::Error::from_contract_error(1001)
            }
            SerializationError::NestingTooDeep => {
                soroban_sdk::Error::from_contract_error(1002)
            }
            SerializationError::InvalidValue => {
                soroban_sdk::Error::from_contract_error(1003)
            }
            SerializationError::EmptyCollection => {
                soroban_sdk::Error::from_contract_error(1004)
            }
            SerializationError::CircularReference => {
                soroban_sdk::Error::from_contract_error(1005)
            }
        }
    }
}
