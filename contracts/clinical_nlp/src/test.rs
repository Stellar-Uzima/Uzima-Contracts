...
fn test_process_clinical_note() {
    ...
    // Process the note
    let result = client.process_clinical_note(
        &note_text,
        &note_id,
        &patient_id,
        &record_id,
...
fn test_analyze_sentiment() {
    ...
    assert!(sentiment.label == sentiment::SentimentLabel::Positive || 
    ...
    let sentiment = result.unwrap();
    assert!(sentiment.score < 0);
    assert!(sentiment.label == sentiment::SentimentLabel::Negative || 
...
fn test_processing_stats() {
    ...
    client.process_clinical_note(&note_text, &note_id, &patient_id, &record_id, &0).unwrap();
