...
impl DicomwebServicesContract {
    ...
    pub fn stow_store_instance(
        ...
        study.patient_id =
            Self::get_json_string(&env, &request.json_metadata, TAG_PATIENT_ID);
        study.patient_name =
            Self::get_json_string(&env, &request.json_metadata, TAG_PATIENT_NAME);
        study.study_description =
            Self::get_json_string(&env, &request.json_metadata, TAG_STUDY_DESCRIPTION);
    ...
    fn get_json_u32(env: &Env, json: &DicomJsonObject, tag: Symbol) -> u32 {
        if let Some(attr) = json.attributes.get(tag) {
            if let Some(val) = attr.value.get(0) {
                ...
                let mut buf = [0u8; 32];
                val.copy_into_slice(&mut buf[..len as usize]);
                let mut n: u32 = 0;
