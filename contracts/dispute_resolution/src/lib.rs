#![no_std]
use soroban_sdk::{contract, contractimpl, Address, Env, Map, Vec, symbol_short};
...
impl DisputeResolution {
    pub fn initialize(env: Env, arbiters: Vec<Address>) {
        env.storage().instance().set(&symbol_short!("arbiters"), &arbiters);
    }
    ...
    pub fn dispute(env: Env, proposal_id: u64, challenger: Address) {
        ...
        let mut disputes: Map<u64, bool> = env.storage().persistent().get(&symbol_short!("disputes")).unwrap_or(Map::new(&env));
        disputes.set(proposal_id, true); // True means active dispute
        env.storage().persistent().set(&symbol_short!("disputes"), &disputes);
    }
    ...
    pub fn resolve(env: Env, proposal_id: u64, arbiter: Address, valid_proposal: bool) {
        ...
        let arbiters: Vec<Address> = env.storage().instance().get(&symbol_short!("arbiters")).unwrap();
        if !arbiters.contains(&arbiter) { panic!("not arbiter"); }

        let mut disputes: Map<u64, bool> = env.storage().persistent().get(&symbol_short!("disputes")).unwrap();
    ...
    pub fn is_disputed(env: Env, proposal_id: u64) -> bool {
        let disputes: Map<u64, bool> = env.storage().persistent().get(&symbol_short!("disputes")).unwrap_or(Map::new(&env));
