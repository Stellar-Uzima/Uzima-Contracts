...
impl FederatedLearningContract {
    pub fn initialize(env: Env, admin: Address, coordinator: Address) -> Result<bool, Error> {
        ...
        env.storage()
            .instance()
            .set(&DataKey::ValidationThreshold, &60u32);
        env.storage()
            .instance()
            .set(&DataKey::MarketplaceFeeBps, &200u32);
        env.storage()
            .instance()
            .set(&DataKey::RareRewardMultiplier, &3u32);
        env.storage().instance().set(&DataKey::MaxEpsilon, &200u32);
        env.storage()
            .instance()
            .set(&DataKey::MinReputation, &10u32);
        Ok(true)
    }
    ...
    pub fn update_config(
        ...
        env.storage()
            .instance()
            .set(&DataKey::ValidationThreshold, &validation_threshold);
        env.storage()
            .instance()
            .set(&DataKey::MarketplaceFeeBps, &fee_bps);
        env.storage()
            .instance()
            .set(&DataKey::RareRewardMultiplier, &rare_multiplier);
        env.storage()
            .instance()
            .set(&DataKey::MaxEpsilon, &max_epsilon);
        env.storage()
            .instance()
    ...
    fn consume_privacy_budget(
        ...
        if b.epsilon_consumed + eps > b.epsilon_total || b.delta_consumed + delta > b.delta_total {
    ...
    pub fn register_rare_disease(
        ...
        env.events()
            .publish((symbol_short!("RareDis"),), disease_code);
        Ok(true)
    }
    ...
    pub fn finalize_round(
        ...
        let reward_mult: i128 = if round.is_rare_disease {
            rare_mult as i128
        } else {
            1
        };
        ...
        if round.is_rare_disease {
            let rd_k = DataKey::RareDiseaseRegistry(round.disease_code.clone());
            if let Some(mut rd) = env
                .storage()
                .persistent()
                .get::<DataKey, RareDiseaseRegistry>(&rd_k)
            {
                rd.model_count += 1;
                rd.participant_count += round.total_updates;
                env.storage().persistent().set(&rd_k, &rd);
            }
        }
        env.events()
...
mod test {
    ...
    fn test_dp_budget_enforcement() {
        ...
        let r1 = client.mock_all_auths().start_round(
            &admin,
            &BytesN::from_array(&env, &[1u8; 32]),
            &default_cfg(&env, 1, 10),
        );
        // 1000 samples, no noise => eps_cost = 1 + 2 = 3, exhausts budget
        assert!(client
            .mock_all_auths()
            .try_submit_update(
                &inst,
            ...
            .unwrap()
            .is_ok());
        let r2 = client.mock_all_auths().start_round(
            &admin,
            &BytesN::from_array(&env, &[1u8; 32]),
            &default_cfg(&env, 1, 10),
        );
    ...
    fn test_marketplace() {
        ...
        assert_eq!(
            client.get_listing(&lid).unwrap().status,
            ListingStatus::Sold
        );
        assert_eq!(
    ...
    fn test_model_types() {
        ...
        .enumerate()
        {
            ...
            do_submit(&client, &env, &inst, round_id, (i + 2) as u8);
            assert!(client.mock_all_auths().begin_aggregation(&coord, &round_id));
            let mid = BytesN::from_array(&env, &[(i + 10) as u8; 32]);
            client.mock_all_auths().finalize_round(
                &coord,
                &round_id,
                &ModelOutput {
            ...
            assert_eq!(client.get_model(&mid).unwrap().model_type, *mt);
        }
    }
}
