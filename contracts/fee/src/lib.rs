...
/// utility so fee tiers stay consistent with batch-rewards and rewards
...
#[contractimpl]
impl FeeContract {
    /// Calculates the applicable fee for `value` using the shared
    /// tiered-rate utility.
    ///
...
mod test {
    ...
    #[test]
    fn fee_calculation_uses_shared_utility() {
        let env = Env::default();
        let tiers = sample_tiers(&env);

        // 500 * 1.00% = 5
        assert_eq!(FeeContract::calculate_tiered_rate(500, tiers.clone()), 5);
        // value == 1000 uses the 0.50% tier: 1000 * 0.50% = 5
        assert_eq!(FeeContract::calculate_tiered_rate(1_000, tiers.clone()), 5);
        // 999 * 1.00% = 9 (integer division)
        assert_eq!(FeeContract::calculate_tiered_rate(999, tiers.clone()), 9);
        // 50_000 * 0.25% = 125
        assert_eq!(FeeContract::calculate_tiered_rate(50_000, tiers.clone()), 125);
        assert_eq!(FeeContract::calculate_tiered_rate(0, tiers), 0);
    }
    ...
    fn matches_shared_implementation_exactly() {
        ...
        for value in [0i128, 1, 500, 999, 1_000, 1_001, 10_000, 50_000] {
            assert_eq!(
                FeeContract::calculate_tiered_rate(value, tiers.clone()),
    ...
    fn empty_tiers_return_zero() {
        let env = Env::default();
        assert_eq!(FeeContract::calculate_tiered_rate(5_000, Vec::new(&env)), 0);
    }
}
