...
impl Fido2AuthenticatorContract {
    ...
    pub fn generate_challenge(env: Env, user: Address) -> Result<BytesN<32>, Error> {
        ...
        let mut tmp = ts;
        for i in 0..8usize {
            ts_buf[7 - i] = (tmp & 0xff) as u8;
            tmp >>= 8;
        }
        let mut tmp32 = seq;
        for i in 0..4usize {
            seq_buf[3 - i] = (tmp32 & 0xff) as u8;
            tmp32 >>= 8;
        }
        ...
        let pending = PendingChallenge {
            ...
            created_at: ts,
            expires_at: ts + CHALLENGE_TTL_SECS,
        };
        env.storage()
            .persistent()
    ...
    pub fn register_ed25519_device(
        ...
        for i in 0..devices.len() {
            let d: Fido2Device = devices.get(i).unwrap();
    ...
    pub fn verify_ed25519_assertion(
        ...
        let device: Fido2Device = devices.get(idx).unwrap();
