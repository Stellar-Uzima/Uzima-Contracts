...
use soroban_sdk::{
    contract, contractimpl, contracttype, symbol_short, vec, Address, Bytes, Env, Map, Symbol, IntoVal
};
...
impl Governor {
    ...
    pub fn propose(
        ...
        env.events().publish((symbol_short!("Propose"), id), proposer);
        id
    }
    ...
    pub fn cast_vote(env: Env, proposal_id: u64, voter: Address, support: u32) {
        ...
        let mut votes: Map<(u64, Address), u32> = env.storage().persistent().get(&VOTES).unwrap_or(Map::new(&env));
        if votes.contains_key(vote_key.clone()) {
            panic!("already voted");
        }

        let weight = Self::get_power(&env, &cfg, &voter);
        if weight == 0 { panic!("no voting power"); }
        ...
        env.events().publish((symbol_short!("Vote"), proposal_id), (voter, support, weight));
    }
    ...
    pub fn state(env: Env, proposal_id: u64) -> u32 {
        ...
        if let Some(dispute_addr) = cfg.dispute_contract {
             ...
             let is_disputed: bool = env.invoke_contract(&dispute_addr, &Symbol::new(&env, "is_disputed"), args);
             if is_disputed { return 6; } // Disputed
        }

        if t < p.start_time { return 0; } 
        if t <= p.end_time { return 1; }  

        if p.for_votes > p.against_votes { return 3; } // Succeeded
    ...
    pub fn queue(env: Env, proposal_id: u64) {
        ...
        if state != 3 { panic!("proposal not successful"); }
    ...
    pub fn execute(env: Env, proposal_id: u64) {
        ...
        if p.executed { panic!("already executed"); }

        let cfg: GovernorConfig = env.storage().instance().get(&CFG).unwrap();
        if let Some(dispute_addr) = cfg.dispute_contract {
             let args = vec![&env, proposal_id.into_val(&env)];
             let is_disputed: bool = env.invoke_contract(&dispute_addr, &Symbol::new(&env, "is_disputed"), args);
             if is_disputed { panic!("proposal disputed"); }
        }
