...
fn test_consent_management() {
    ...
    // Grant consent
    let result =
        HealthcareComplianceContract::grant_consent(env.clone(), patient.clone(), consent.clone());
    assert!(result.is_ok());
...
fn test_audit_logging() {
    ...
    // Get audit logs
    let logs =
        HealthcareComplianceContract::get_audit_logs(env.clone(), doctor.clone(), 10).unwrap();
    ...
    assert_eq!(log_entry.hipaa_category, 1);
}
...
fn test_breach_reporting() {
    ...
    let breach = BreachReport {
        ...
        mitigation_steps: Vec::from_array(
            &env,
            [
    ...
    // Report breach
    let result =
        HealthcareComplianceContract::report_breach(env.clone(), reporter.clone(), breach.clone());
    assert!(result.is_ok());
...
fn test_admin_functions() {
    ...
    let result2 =
        HealthcareComplianceContract::grant_consent(env.clone(), non_admin.clone(), consent);
    assert!(result2.is_err());
...
fn test_consent_expiration() {
    ...
    let result =
        HealthcareComplianceContract::grant_consent(env.clone(), patient.clone(), expired_consent);
...
fn test_invalid_consent_operations() {
    ...
    let result = HealthcareComplianceContract::revoke_consent(
        env.clone(),
        other_patient.clone(),
        String::from_str(&env, "consent_001"),
...
fn test_compliance_score_updates() {
    ...
    let initial_score = HealthcareComplianceContract::get_compliance_metrics(env.clone())
        .unwrap()
        .compliance_score;
    ...
    let new_score = HealthcareComplianceContract::get_compliance_metrics(env.clone())
        .unwrap()
        .compliance_score;
