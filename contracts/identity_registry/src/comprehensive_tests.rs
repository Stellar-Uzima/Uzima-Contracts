...
fn test_verify_did_authorization_multiple_relationships() {
    ...
    assert!(client.verify_did_authorization(&subject, &VerificationRelationship::CapabilityDelegation));
}
...
fn test_add_fido2_device_eddsa() {
    ...
    assert!(matches!(fido2_vm.method_type, VerificationMethodType::Fido2EdDsa2024));
}
...
fn test_add_fido2_device_es256() {
    ...
    let did_doc = client.resolve_did(&subject);
    let fido2_vm = did_doc.verification_methods.get(1).unwrap();
    assert!(matches!(fido2_vm.method_type, VerificationMethodType::Fido2Es2562024));
}
...
fn test_multiple_subjects_isolation() {
    ...
    assert_ne!(creds1.get(0).unwrap().credential_type, creds2.get(0).unwrap().credential_type);
}
