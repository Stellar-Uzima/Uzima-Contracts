...
pub fn publish_record_stored(env: &Env, patient_id: &Address, record_hash: &BytesN<32>, timestamp: u64) {
    env.events().publish(
...
pub fn publish_record_verified(env: &Env, patient_id: &Address, record_hash: &BytesN<32>, verified: bool) {
    env.events().publish(
