...
use soroban_sdk::{vec, String, testutils::{Ledger, Address as _}};
...
fn test_emergency_pause_and_resume() {
    ...
    let result = t.client.try_add_record(
        &t.doctor, &t.patient, &String::from_str(&env, "D"), &String::from_str(&env, "T"), 
        &false, &vec![&env], &String::from_str(&env, "Modern"), 
        &String::from_str(&env, "T"), &String::from_str(&env, "ipfs://cid123456")
    );
...
fn test_performance_and_pagination() {
    ...
    for _ in 0..20 {
        t.client.add_record(
            &t.doctor, &t.patient, &String::from_str(&env, "Diag"), &String::from_str(&env, "Treat"), 
            &false, &vec![&env], &String::from_str(&env, "Modern"), 
            &String::from_str(&env, "T"), &String::from_str(&env, "ipfs://cid123456")
        );
    }
...
fn test_did_and_identity_management() {
    ...
    t.client.set_did_auth_level(&t.admin1, &medical_records::DIDAuthLevel::Basic);
    assert!(matches!(t.client.get_did_auth_level(), medical_records::DIDAuthLevel::Basic));

    // 3. User links their own DID (Coverage: link_did_to_user)
    t.client.link_did_to_user(&t.patient, &t.patient, &patient_did);
...
fn test_ai_config_and_risk_submission() {
    ...
    let risk_insight = t.client.get_latest_risk_score(&t.patient, &t.patient).unwrap();
    ...
    let result = t.client.try_get_latest_risk_score(&unauthorized, &t.patient);
...
fn test_emergency_access_and_logging() {
    ...
    let has_access = t.client.has_emergency_access(&emergency_provider, &t.patient, &1);
    assert_eq!(has_access, true);

    // 3. Patient revokes access (Coverage: revoke_emergency_access)
    t.client.revoke_emergency_access(&t.patient, &emergency_provider);
    ...
    t.client.manage_user(&t.admin1, &t.doctor, &Role::Doctor);
    t.client.add_record(&t.doctor, &t.patient, &String::from_str(&env, "Diag"), &String::from_str(&env, "Treat"), &false, &vec![&env], &String::from_str(&env, "Modern"), &String::from_str(&env, "T"), &String::from_str(&env, "ipfs://cid1234567890"));

    // Call get_record_with_did to force log_access
    let _ = t.client.try_get_record_with_did(&t.admin1, &1, &String::from_str(&env, "Audit"));
    
    // Check patient's log has at least 1 entry
    let logs = t.client.get_patient_access_logs(&t.admin1, &t.patient, &0, &10);
    assert!(logs.len() > 0);
}
...
fn test_cross_chain_workflow() {
    ...
    t.client.register_cross_chain_ref(&t.patient, &1, &chain_id, &external_hash);

    // 3. Admin updates sync status (Coverage: update_cross_chain_sync)
    let new_hash = soroban_sdk::BytesN::from_array(&env, &[3; 32]);
    t.client.update_cross_chain_sync(&t.admin1, &1, &chain_id, &new_hash);
    ...
    let result = t.client.try_get_record_cross_chain(&bridge, &1, &chain_id, &String::from_str(&env, "0xaccess"));
    assert!(result.is_ok());
}