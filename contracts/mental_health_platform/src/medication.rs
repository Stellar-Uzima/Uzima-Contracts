...
impl MedicationManager {
    pub fn create_medication_plan(
        ...
        let plan = MedicationPlan {
            ...
            start_date,
            end_date,
            prescribed_by: prescribed_by.clone(),
            side_effects,
        ...
        env.events().publish(
            ...
            (plan_id, patient_id.clone(), prescribed_by.clone()),
        );
        ...
        for plan in plans.iter() {
            if plan.plan_id == plan_id {
                ...
                for entry in plan.adherence_tracking.iter() {
                    if env.ledger().timestamp() - entry.timestamp < days as u64 * 24 * 60 * 60 {
                ...
                let taken_count = recent_entries.iter().filter(|entry| entry.taken).count() as u32;

                return Ok(taken_count * 100 / recent_entries.len() as u32);
            }
        }
    ...
    pub fn analyze_medication_effectiveness(
        ...
        let plans = Self::get_patient_medication_plans(env, patient_id.clone());

        for plan in plans.iter() {
            if plan.plan_id == plan_id {
                let adherence_rate = Self::calculate_adherence_rate(env, plan_id, patient_id.clone(), 30)?;
                let effectiveness_rating = plan.effectiveness_rating.unwrap_or(5); // Rating is 0-10
                ...
                for (_side_effect, _count) in side_effect_counts.iter() {
                ...
                let recommendations = Self::generate_medication_recommendations(
                    ...
                    side_effects_frequency.len() as usize,
                );
    ...
    fn check_adherence_patterns(env: &Env, patient_id: Address, plan: MedicationPlan) {
        let adherence_rate =
            Self::calculate_adherence_rate(env, plan.plan_id, patient_id.clone(), 7).unwrap_or(0);

        if adherence_rate < 70 {
            // Low adherence detected (less than 70%)
            env.events().publish(
                (Symbol::new(env, "low_adherence_alert"),),
                (plan.plan_id, patient_id.clone(), adherence_rate),
            );
        }

        // Check for side effect patterns
        let mut recent_side_effects = Vec::new(env);
        for entry in plan.adherence_tracking.iter() {
            if env.ledger().timestamp() - entry.timestamp < 7 * 24 * 60 * 60 {
                for side_effect in entry.side_effects_experienced.iter() {
                    recent_side_effects.push_back(side_effect);
                }
            }
        }
    ...
    fn generate_medication_recommendations(
        ...
        if adherence_rate < 80 { // Less than 80% adherence
