...
### Categorization Snapshot (last updated: fix/issues-1442-1443-1444-1445-reintegrate-clinical-emr)
...
| **Fix & Include** (reintegrated, issues #1429/#1430/#1431) | `explainable_ai` | Manifest converted to `workspace = true`, standalone `cargo check`/`cargo test` verified; blocked from a `--workspace`-wide verification only by the unrelated pre-existing issue described below |
| **Standalone regression coverage only, still excluded by design** (issue #1433) | `sut_token` | Not reintegrated (out of scope for #1433); manifest made standalone-buildable and `cargo test --manifest-path contracts/sut_token/Cargo.toml` passes (20/20 tests) |
| **Reintegrated** (issue #1426) | `deprecation_framework` | Removed from the root workspace `exclude` list; manifest resolves `soroban-sdk` (with the `alloc` feature) and `governance_commons` strictly through the workspace `=21.7.7` policy; `rlib` added to crate types for the workspace test harness; public types/entrypoints unchanged. Covered by `.github/workflows/deprecation-framework-check.yml` |
| **Deferred** (out of scope for current PR) | `health_data_access_logging` (re-deferred this PR due to 14 `#[no_std]`/SDK-21 incompatibilities: missing `format!` macro, `Vec::with_capacity` not on `soroban_sdk::Vec`, wrong `BytesN::from_array` arg shape, missing `Copy` derives), `medical_imaging`, `healthcare_compliance`, `clinical_nlp`, `remote_patient_monitoring`, `healthcare_analytics_dashboard`, `healthcare_data_marketplace`, `telemedicine`, `medical_imaging_ai`, `dicomweb_services`, `multi_region_orchestrator`, `regional_node_manager`, `digital_twin`, `aml`, `forensics`, `federated_learning`, `healthcare_oracle_network` | Tracked in `Cargo.toml` `exclude` block with rationale; follow-up PR per contract |
| **Deferred** (out of scope for current PR) | `health_data_access_logging` (re-deferred this PR due to 14 `#[no_std]`/SDK-21 incompatibilities: missing `format!` macro, `Vec::with_capacity` not on `soroban_sdk::Vec`, wrong `BytesN::from_array` arg shape, missing `Copy` derives), `medical_imaging`, `healthcare_compliance`, `clinical_nlp`, `remote_patient_monitoring`, `healthcare_analytics_dashboard`, `healthcare_data_marketplace`, `telemedicine`, `medical_imaging_ai`, `multi_region_orchestrator`, `regional_node_manager`, `digital_twin`, `aml`, `forensics`, `federated_learning`, `medical_records`, `healthcare_oracle_network` | Tracked in `Cargo.toml` `exclude` block with rationale; follow-up PR per contract |
| **Deferred** (out of scope for current PR) | `health_data_access_logging` (re-deferred this PR due to 14 `#[no_std]`/SDK-21 incompatibilities: missing `format!` macro, `Vec::with_capacity` not on `soroban_sdk::Vec`, wrong `BytesN::from_array` arg shape, missing `Copy` derives), `medical_imaging`, `healthcare_compliance`, `clinical_nlp`, `remote_patient_monitoring`, `healthcare_analytics_dashboard`, `healthcare_data_marketplace`, `telemedicine`, `medical_imaging_ai`, `dicomweb_services`, `multi_region_orchestrator`, `regional_node_manager`, `aml`, `forensics`, `federated_learning`, `medical_records`, `healthcare_oracle_network` | Tracked in `Cargo.toml` `exclude` block with rationale; follow-up PR per contract |
